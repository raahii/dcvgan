...
class TestModelForward(unittest.TestCase):
    ...
    def test_gradient_discriminator(self):
        inputs = {
            "ch1": GEOMTRIC_INFO_CH,
            "ch2": COLOR_CH,
            "use_noise": True,
        ...
        gdis = GradientDiscriminator(**inputs)

        xg_shape = (BATCHSIZE, GEOMTRIC_INFO_CH, VIDEO_LENGTH, IMAGE_SIZE, IMAGE_SIZE)
        xg = torch.empty(xg_shape, device=gdis.device).normal_()

        xc_shape = (BATCHSIZE, COLOR_CH, VIDEO_LENGTH, IMAGE_SIZE, IMAGE_SIZE)
        xc = torch.empty(xc_shape, device=gdis.device).normal_()
